...
export default function LojaHero({ bannersUrls }) {
  const [idx, setIdx] = useState(0);

  // Se há banners reais do banco (imagens), usa; senão usa os padrão com gradiente
  const usandoImagens = bannersUrls && bannersUrls.length > 0;
  const total = usandoImagens ? bannersUrls.length : BANNERS_PADRAO.length;
  ...
  if (usandoImagens) {
    return (
      <div className="relative w-full aspect-[3/1] md:aspect-[5/1] rounded-2xl overflow-hidden shadow-xl bg-slate-200">
        <img
          src={bannersUrls[safeIdx]}
          alt={`Banner ${safeIdx + 1}`}
          className="w-full h-full object-cover transition-all duration-500"
        />
        <button onClick={() => setIdx(i => (i - 1 + total) % total)}
          className="absolute left-3 top-1/2 -translate-y-1/2 bg-white/30 hover:bg-white/60 backdrop-blur-sm rounded-full p-2 transition-all">
          <ChevronLeft className="w-5 h-5 text-white" />
        </button>
        <button onClick={() => setIdx(i => (i + 1) % total)}
          className="absolute right-3 top-1/2 -translate-y-1/2 bg-white/30 hover:bg-white/60 backdrop-blur-sm rounded-full p-2 transition-all">
          <ChevronRight className="w-5 h-5 text-white" />
        </button>
        <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2">
          {bannersUrls.map((_, i) => (
            <button key={i} onClick={() => setIdx(i)}
              className={cn("h-2 rounded-full transition-all", i === safeIdx ? "w-6 bg-white" : "w-2 bg-white/40")} />
          ))}
        </div>
      </div>
    );
  ...
  const banner = BANNERS_PADRAO[safeIdx];
